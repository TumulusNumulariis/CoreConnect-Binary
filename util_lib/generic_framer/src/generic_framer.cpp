...
generic_framer::generic_framer(unsigned int number_of_streams, unsigned short max_rx_frame_length)
    {
...
int generic_framer::parse_raw_rx_data(unsigned int stream, const char *src, unsigned int len)
    {
    STREAM_CONTROL *sc;
    ...
    while (len && ret == E_frmr_no_error)
        ...
        case frame_state_hdr2:
            ...
            sc->m_required_len -= sizeof(unsigned short);
