...
// Frame is: <2 byte binary frame len> <variable len payload>
// length is total length of the payload but does include the 2 byte frame length itself
// so min frame length is 2, max frame length is 65535
// min length of a body is 0, max len of a body is 65533
...
class generic_framer
    {
public:
    generic_framer(unsigned int number_of_streams, unsigned short max_rx_frame_size);
    virtual ~generic_framer();
    enum framer_errors
        {
        E_frmr_no_error = 0,
    ...
    static const unsigned int sm_min_frame_length = 2U;
    static const unsigned int sm_max_frame_length = 65536U;
    static const unsigned int sm_max_number_streams = 100;
private:
    generic_framer(const generic_framer &) = delete;
    generic_framer &operator=(const generic_framer &) = delete;
...
inline int generic_framer::clear(unsigned int stream)
    {
...
inline void generic_framer::clear_stream(unsigned int stream)
    {
